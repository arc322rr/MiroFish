          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Диалог с любым агентом мира' }}</span>
            ...
            >
              ...
              <span>Отправить опрос в симулируемый мир</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Быстрый диалоговый режим агента отчетов: 4 профессиональных инструмента и полный доступ к памяти MiroFish</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge Глубинная атрибуция</div>
                    <div class="tool-desc">Сопоставляет исходные данные и состояние симуляции, используя Global/Local Memory для причинного анализа</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch Панорамный трекинг</div>
                    <div class="tool-desc">Восстанавливает пути распространения событий на графе и фиксирует полную топологию потоков информации</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Мгновенный интерфейс поиска на базе GraphRAG для извлечения свойств узлов и отдельных фактов</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent Виртуальные интервью</div>
                    <div class="tool-desc">Параллельные многошаговые интервью с агентами мира для сбора неструктурированных мнений и состояния</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Диалог с Report Agent для более глубокого анализа отчета' : 'Диалог с агентом симуляции для изучения его позиции' }}
              </p>
            </div>
            <div 
...
const sendToReportAgent = async (message) => {
  addLog(`Отправлено Report Agent: ${message.substring(0, 50)}...`)
...
const sendToAgent = async (message) => {
  ...
  addLog(`Отправлено ${selectedAgent.value.username}: ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Пользователь' : 'Вы'}: ${msg.content}`)
      .join('\n')
...
const submitSurvey = async () => {
  ...
  addLog(`Отправка опроса ${selectedAgents.value.size} респондентам...`)
  ...
  try {
    ...
    addLog(`Ошибка отправки опроса: ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    ...
    addLog(`Ошибка загрузки отчета: ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    addLog(`Ошибка загрузки логов отчета: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    addLog(`Ошибка загрузки агентов симуляции: ${err.message}`)
  }
}
...
// Lifecycle
onMounted(() => {
  addLog('Инициализация Step5: глубокое взаимодействие')
  loadReportData()
  loadProfiles()

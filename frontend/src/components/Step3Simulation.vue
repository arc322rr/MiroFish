    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Запуск...' : 'Сгенерировать итоговый отчет' }} 
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog('Запуск параллельной симуляции на двух платформах...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Старые логи симуляции очищены, запуск с начала')
      }
      addLog('✓ Движок симуляции успешно запущен')
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Не удалось получить детальный статус:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Запрос на генерацию отчета отправлен, ожидайте...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Запуск генерации отчета...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Задача генерации отчета запущена: ${reportId}`)
    ...
    addLog(`✗ Исключение при запуске генерации отчета: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Инициализация Step3: выполнение симуляции')
  if (props.simulationId) {
    doStartSimulation()
  }
})
